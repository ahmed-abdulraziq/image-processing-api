...
resizeImage.get("/", (req: express.Request, res: express.Response) => {
    ...
    const pathImage: string = `${path.resolve(__dirname,`../../img/${filename}.jpg`)}`;
    const pathThumbImage: string = `${path.resolve(__dirname,`../../img/thumb/${filename}-${height}x${width}.jpg`)}`;
